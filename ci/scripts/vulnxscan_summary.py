...
aggregate / delta / identity / tracker と同じく `def main(argv)` + `if __name__` ガードを持つ
(17d7fd6 で他 4 スクリプトが統一済)。分類 (classify) / closure グラフ (Closure) / notify payload
生成 (build_notify_payload) を関数・クラスに切り出してあるので、#285 系分岐 (identity-mismatch /
reclassified / spot-check / pin 検出) をユニットテストから直接 import して検証できる
(test_summary.py 参照)。CLI の位置引数順 (csv/target/closure/notify/roots/tracker/identity) は
scan-vulnerabilities.yaml の caller が依存するため不変。
"""
import csv
import json
import re
...
# tracker で no-fix/UNKNOWN を 🔁 reclassified に降格する status (#285c/#289)。
...
class Closure:
    """closure (`nix path-info -r [--json]`) + provenance (vulnxscan_provenance.nix) を保持し、
    pin 検出 / 由来 (bundled by) / 入口 (declared root) 逆引きを提供する。

    旧 module-level グローバル (closure_versions / ref_parents / store_paths / declared_roots) を
    インスタンス状態に閉じ込め、import 時副作用を排して直接ユニットテスト可能にする (T55)。
    closure / roots を load しなければ全メソッドが空 (loaded=False) として振る舞う。"""

    def __init__(self):
        self.closure_versions = defaultdict(set)  # name -> {versions} (pin 検出)
        self.ref_parents = {}  # store path -> {それを参照する store path}
        self.store_paths = []  # closure 内の全 store path (--json 時のみ)
        self.declared_roots = {}  # outPath -> (name, file, src)

    @property
    def loaded(self):
        return bool(self.closure_versions)

    def load_closure(self, path):
        ...
        if text.lstrip()[:1] in "[{":
            ...
            for p, refs in graph:
                if not p:
                    continue
                self.store_paths.append(p)
                n, ver = _node_name(p)
                if ver:
                    self.closure_versions[n].add(ver)
                for r in refs:
                    self.ref_parents.setdefault(r, set()).add(p)
            return bool(self.store_paths)
        ...
        for line in text.splitlines():
            ...
            if m:
                self.closure_versions[m.group(1)].add(m.group(2))
        return bool(self.closure_versions)

    def load_roots(self, path):
        try:
            with open(path) as f:
                data = json.load(f)
        ...
        for e in data:
            o = e.get("o")
            if o:
                self.declared_roots.setdefault(o, (e.get("n", "?"), e.get("f", ""), e.get("src", "")))
        return bool(self.declared_roots)

    def is_pinned(self, pkg):
        """package が closure に複数版で存在 = 意図的 pin (古い版の CVE は auto-update で直らない)。"""
        return len(self.closure_versions.get(pkg, set())) >= 2

    @lru_cache(maxsize=None)
    def bundled_by(self, pkg, ver, cap=3):
        """pkg@ver を closure に入れた『意味ある親』(由来)。generic ノードと自身の
        output (同名) を除外し、上位 cap 件 + 残数を返す。参照グラフが無ければ ""。
        直接導入 (generic な親しか居ない) は "—"。"""
        if not self.ref_parents:
            return ""
        ...
        for p in self.store_paths:
            if ver:
                ...
                continue
            for parent in self.ref_parents.get(p, ()):
    ...
    def entry_points(self, pkg, ver):
        ...
        if not self.declared_roots or not self.ref_parents:
            return None
        targets = self._vuln_paths(pkg, ver)
        seen = set(targets)
        q = deque(targets)
        found = {}
        while q:
            cur = q.popleft()
            if cur in self.declared_roots:
                n, f, s = self.declared_roots[cur]
                found[(_pkgname(n), _relfile(f), s)] = 1
            for parent in self.ref_parents.get(cur, ()):
                if parent not in seen:
                    seen.add(parent)
                    q.append(parent)
        return sorted(found.keys())

    def origin(self, pkg, ver):
        """表示用の『入口 (設定)』文字列。provenance があれば入口+ファイル、
        多すぎる (基盤依存) なら縮退、無ければ由来 (bundled by) にフォールバック。"""
        eps = self.entry_points(pkg, ver)
        if eps is None:
            return self.bundled_by(pkg, ver)  # フォールバック (closure immediate referrer)
        if not eps:
            return "—"
...
def load_csv(csv_path):
    """triage CSV を読む。存在しなければ None (= スキャン失敗の可能性) を返す。"""
    try:
...
def _load_json_map(path):
    """JSON マップ ({k: v}) を読み込む。無い/壊れていれば空 dict。"""
    if not path:
        return {}
    try:
        with open(path) as f:
...
def classify(rows, closure, tracker_status, identity_map):
    """triage rows を #285 のバケツに振り分ける。

    closure (Closure): pin 検出 (fixable vs info)。loaded=False なら全て info 落ち。
    tracker_status ({cve: status}): no-fix/UNKNOWN を notaffected/notforus で reclassified へ降格。
    identity_map ({vuln_id: {verdict,...}}): collision を除外、affected を judged へ昇格、
      disputed/not_in_range を likely_fp へ降格 (#289)。

    返り値: バケツ名 -> row list の dict と、tracker/identity が有効だったかの flag。
    """
    ...
    for r in rows:
        ...
        if cl in UNKNOWN_CLASSIFY:
            ...
            if closure.loaded and closure.is_pinned(r.get("package", "")):
                fixable.append(r)
            else:
                info.append(r)
    ...
    # repology 単独で出した no-fix / UNKNOWN を nixpkgs の権威ステータスで見直す。
    # notaffected/notforus (backport patch / 対象外) は silent DROP せず 🔁 reclassified に
    # 降格して「要確認・whitelist 候補」として可視化 (保守的)。affected/wontfix/unknown は
    # 据え置き、各 finding に _tracker を付けて nixpkgs 列に注記。
    ...
    def _identity_filter(bucket):
        ...
        for r in bucket:
            vid = r.get("vuln_id", "")
            info_e = identity_map.get(vid)
            if info_e:
                r["_identity"] = info_e  # 移動有無に関わらず注記 (nofix_cpe は keep 側で表示に使う)
            dest = dest_of.get((info_e or {}).get("verdict"))
            if dest is not None:
    ...
    return {
        ...
        "judged": judged,
        "likely_fp": likely_fp,
    }, tracker_loaded, identity_loaded


def build_notify_payload(target, buckets, closure):
    """集約 (Issue 起票) / delta 用の notify.json payload を組み立てる。
    後方互換: 既存の "findings" (NOTIFY) キーは不変。unknown/spotcheck 等を追加キーとして並べる
    (古い aggregate は未知キーを無視するだけ) → 集約 Issue でも surface (#285a/b)。"""
...
def render_summary(target, rows, buckets, closure, tracker_loaded, identity_loaded):
    """job summary (markdown) を stdout に出力する。"""
    fixable = buckets["fixable"]
    nofix = buckets["nofix"]
    info = buckets["info"]
    drop = buckets["drop"]
    whitelisted = buckets["whitelisted"]
    unknown = buckets["unknown"]
    spotcheck = buckets["spotcheck"]
    reclassified = buckets["reclassified"]
    collision = buckets["collision"]
    judged = buckets["judged"]
    likely_fp = buckets["likely_fp"]
    ...
    print(
        ...
        + ("" if closure.loaded else "  _(closure 未指定: pin 検出 OFF)_")
    ...
    def _nofix_cpe(r):
        ...
        info_e = r.get("_identity") or {}
        if info_e.get("verdict") != "nofix_cpe":
            return "—"
        kind, detail = info_e.get("kind"), info_e.get("detail", "")
        if kind == "confirmed":
    ...
    def _fp_reason(r):
        ...
        info_e = r.get("_identity") or {}
        v = info_e.get("verdict")
        if v == "disputed":
            st = info_e.get("status", "")
            tag = info_e.get("reason", "")
            return f"NVD tag: {tag}" + (f" ({st})" if st and st.lower() == "rejected" else "")
        if v == "not_in_range":
            return f"版範囲外 {info_e.get('cpe', '')} {info_e.get('range', '')}".strip()
        return ""
...
def main(argv):
    # CLI の位置引数順は scan-vulnerabilities.yaml の caller が依存するため不変 (T55)。
    csv_path = argv[1] if len(argv) > 1 else "vulns.triage.csv"
    target = argv[2] if len(argv) > 2 else "(unknown)"
    closure_path = argv[3] if len(argv) > 3 else None
    notify_json_path = argv[4] if len(argv) > 4 else None
    roots_path = argv[5] if len(argv) > 5 else None
    tracker_path = argv[6] if len(argv) > 6 else None
    identity_path = argv[7] if len(argv) > 7 else None
    ...
    closure = Closure()
    if closure_path and not closure.load_closure(closure_path):
        closure_path = None
    if roots_path and not closure.load_roots(roots_path):
        roots_path = None

    tracker_status = _load_json_map(tracker_path)
    identity_map = _load_json_map(identity_path)

    buckets, tracker_loaded, identity_loaded = classify(rows, closure, tracker_status, identity_map)

    if notify_json_path:
        payload = build_notify_payload(target, buckets, closure)
        with open(notify_json_path, "w") as jf:
            json.dump(payload, jf, ensure_ascii=False)

    render_summary(target, rows, buckets, closure, tracker_loaded, identity_loaded)
    return 0
